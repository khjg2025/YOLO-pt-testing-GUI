...
from PyQt5.QtWidgets import (
    ...
    QGroupBox, QGridLayout, QFormLayout, QMessageBox, QSlider, QSplitter
)
...
class DetectWorker(QThread):
    ...
    def run(self):
        try:
            ...
            classes = self.params.get('classes', None)  # 可能为None或列表
            half = self.params.get('half', False)
            augment = self.params.get('augment', False)
            stream = self.params.get('stream', False)  # 我们不用stream，直接predict
            ...
            # 结果缓存列表：每个元素为一个字典，包含图片名，model1图像，model2图像，信息
            results_cache = []
...
class ExportWorker(QThread):
    ...
    def run(self):
        try:
            ...
            model_path = self.params.get('model_path')
            export_format = self.params.get('format', 'onnx')  # 'onnx' or 'engine'
            half = self.params.get('half', False)
            device = self.params.get('device', 'cpu')
            imgsz = self.params.get('imgsz', 640)
...
class DetectTab(QWidget):
    ...
    def init_ui(self):
        ...
        checkbox_layout.addWidget(self.check_half)
...
class MainWindow(QMainWindow):
    ...
    def closeEvent(self, event):
        self.save_config()
        # 如果有线程运行，询问是否退出
        if (self.detect_tab.detect_worker and self.detect_tab.detect_worker.isRunning()) or \
           (self.train_tab.train_worker and self.train_tab.train_worker.isRunning()) or \
           (self.format_tab.export_worker and self.format_tab.export_worker.isRunning()):
    ...
    def save_config(self):
        config = {
            'detect': {
                ...
                'half': self.detect_tab.check_half.isChecked(),
    ...
    def load_config(self):
        ...
        try:
            ...
            self.detect_tab.check_half.setChecked(detect_cfg.get('half', False))
