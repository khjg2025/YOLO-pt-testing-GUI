...
try:
    ...
    print("警告: 未安装ultralytics库，请使用 pip install ultralytics 安装")

# 尝试导入torch，用于获取CUDA设备
try:
    import torch
...
class DetectWorker(QThread):
    ...
    image_update = pyqtSignal(object, object, int)  # model1_img, model2_img, index
    info_update = pyqtSignal(str)                   # 检测结果信息
    finished_detect = pyqtSignal(list)              # 结果列表（用于缓存）
    ...
    def run(self):
        try:
            ...
            batch_mode = self.params.get('batch_mode')      # True=批量, False=单张
            detection_mode = self.params.get('detection_mode')  # 'single', 'single_label', 'dual'
...
class DetectTab(QWidget):
    ...
    def init_ui(self):
        ...
        btn_model1 = QPushButton("选择模型")
        btn_model1.clicked.connect(lambda: self.select_model(1))
        model1_layout.addWidget(btn_model1)
        ...
        btn_model2 = QPushButton("选择模型")
        btn_model2.clicked.connect(lambda: self.select_model(2))
        model2_layout.addWidget(btn_model2)
        ...
        btn_label = QPushButton("选择标签目录")
        btn_label.clicked.connect(self.select_label_dir)
        ...
        batch_group = QButtonGroup(self)
        batch_group.addButton(self.radio_batch)
        batch_group.addButton(self.radio_single)
        ...
        # 检查模式单选
        self.radio_mode_single = QRadioButton("单模型检查")
        self.radio_mode_single_label = QRadioButton("单模型＋标签")
        self.radio_mode_dual = QRadioButton("双模型检查")
        self.radio_mode_single.setChecked(True)
        mode_group = QButtonGroup(self)
        mode_group.addButton(self.radio_mode_single)
        mode_group.addButton(self.radio_mode_single_label)
        mode_group.addButton(self.radio_mode_dual)
        ...
        # 新增 quantize 数值输入框（替代原 half 复选框）
        self.spin_quantize = QSpinBox()
        self.spin_quantize.setRange(0, 255)
        self.spin_quantize.setValue(16)
        self.spin_quantize.setToolTip("量化参数（0表示不量化）")
    ...
    def prev_image(self):
        if self.img_files and self.current_index > 0:
            # 优先显示缓存中的结果，如果没有则显示原图
    ...
    def next_image(self):
        if self.img_files and self.current_index < len(self.img_files) - 1:
            if self.results_cache:
    ...
    def get_detection_params(self):
        params = {
            ...
            'quantize': self.spin_quantize.value(),  # 新增
...
class TrainTab(QWidget):
    ...
    def init_ui(self):
        ...
        self.edit_model_path.setPlaceholderText("未加载模型/模型地址")
        model_layout.addWidget(QLabel("模型路径:"))
        model_layout.addWidget(self.edit_model_path)
        ...
        btn_yaml = QPushButton("选择配置")
        btn_yaml.clicked.connect(self.select_yaml)
        data_layout.addWidget(btn_yaml)
        ...
        self.spin_imgsz.setRange(32, 4096)
        self.spin_imgsz.setValue(640)
        project_form.addRow("图片尺寸:", self.spin_imgsz)
        ...
        for dev in get_available_devices():
            self.combo_device.addItem(dev)
        project_form.addRow("设备:", self.combo_device)
        ...
        btn_default.clicked.connect(self.preset_default)
        btn_aggressive = QPushButton("激进增强")
        btn_aggressive.clicked.connect(self.preset_aggressive)
        btn_light = QPushButton("轻量训练")
        btn_light.clicked.connect(self.preset_light)
        preset_layout.addWidget(btn_default)
        preset_layout.addWidget(btn_aggressive)
        preset_layout.addWidget(btn_light)
    ...
    def import_config(self):
        ...
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                params = json.load(f)
            self.edit_model_path.setText(params.get('model_path', ''))
...
class FormatTab(QWidget):
    ...
    def init_ui(self):
        ...
        format_group_btn = QButtonGroup(self)
        format_group_btn.addButton(self.radio_onnx)
        format_group_btn.addButton(self.radio_engine)
        ...
        param_layout.addRow("half:", self.check_half)

        self.combo_device = QComboBox()
        for dev in get_available_devices():
            self.combo_device.addItem(dev)
        param_layout.addRow("device:", self.combo_device)

        self.spin_imgsz = QSpinBox()
        self.spin_imgsz.setRange(32, 4096)
        self.spin_imgsz.setValue(640)
        param_layout.addRow("imgsz:", self.spin_imgsz)
...
class MainWindow(QMainWindow):
        ...
        if os.path.exists(self.UI_FILE):
            try:
                ...
                if self.tabs is None:
                    raise Exception("UI文件中未找到名为 'tabs' 的 QTabWidget")
                # 获取标签页对象（索引顺序需与 .ui 中一致）
                self.detect_tab = self.tabs.widget(0)
                self.train_tab = self.tabs.widget(1)
                self.format_tab = self.tabs.widget(2)
            except Exception as e:
                print(f"加载UI文件失败: {e}，回退到代码构建")
                self._build_ui_from_code()
        else:
            self._build_ui_from_code()
    ...
    # ---------- 辅助函数：动态查找控件 ----------
    def _get_child(self, parent, widget_type, name):
        """在 parent 中查找指定类型和名称的控件"""
        return parent.findChild(widget_type, name)

    def _set_lineedit(self, parent, name, value):
        widget = self._get_child(parent, QLineEdit, name)
        if widget:
            widget.setText(value)

    def _set_spinbox(self, parent, name, value):
        widget = self._get_child(parent, QSpinBox, name)
        if widget:
            widget.setValue(value)

    def _set_doublespinbox(self, parent, name, value):
        widget = self._get_child(parent, QDoubleSpinBox, name)
        if widget:
            widget.setValue(value)

    def _set_checkbox(self, parent, name, checked):
        widget = self._get_child(parent, QCheckBox, name)
        if widget:
            widget.setChecked(checked)

    def _set_combobox(self, parent, name, text):
        widget = self._get_child(parent, QComboBox, name)
        if widget:
            idx = widget.findText(text)
            if idx >= 0:
                widget.setCurrentIndex(idx)

    def _get_lineedit_text(self, parent, name, default=''):
        widget = self._get_child(parent, QLineEdit, name)
        return widget.text() if widget else default

    def _get_spinbox_value(self, parent, name, default=0):
        widget = self._get_child(parent, QSpinBox, name)
        return widget.value() if widget else default

    def _get_doublespinbox_value(self, parent, name, default=0.0):
        widget = self._get_child(parent, QDoubleSpinBox, name)
        return widget.value() if widget else default

    def _get_checkbox_checked(self, parent, name, default=False):
        widget = self._get_child(parent, QCheckBox, name)
        return widget.isChecked() if widget else default

    def _get_combobox_text(self, parent, name, default='cpu'):
        widget = self._get_child(parent, QComboBox, name)
        return widget.currentText() if widget else default

    # ---------- 配置保存与加载 ----------
    def save_config(self):
        # 仅当存在 detect_tab 等属性时保存
        if not hasattr(self, 'detect_tab') or not hasattr(self, 'train_tab') or not hasattr(self, 'format_tab'):
            return
        try:
            config = {
                'detect': {
                    'model1': self._get_lineedit_text(self.detect_tab, 'edit_model1'),
                    'model2': self._get_lineedit_text(self.detect_tab, 'edit_model2'),
                    'img_dir': self._get_lineedit_text(self.detect_tab, 'edit_img_path'),
                    'label_dir': self._get_lineedit_text(self.detect_tab, 'edit_label_path'),
                    'conf': self._get_doublespinbox_value(self.detect_tab, 'spin_conf', 0.25),
                    'iou': self._get_doublespinbox_value(self.detect_tab, 'spin_iou', 0.45),
                    'imgsz': self._get_spinbox_value(self.detect_tab, 'spin_imgsz', 640),
                    'max_det': self._get_spinbox_value(self.detect_tab, 'spin_max_det', 300),
                    'classes': self._get_lineedit_text(self.detect_tab, 'edit_classes'),
                    'quantize': self._get_spinbox_value(self.detect_tab, 'spin_quantize', 0),
                    'augment': self._get_checkbox_checked(self.detect_tab, 'check_augment'),
                    'stream': self._get_checkbox_checked(self.detect_tab, 'check_stream'),
                    'batch_mode': self._get_checkbox_checked(self.detect_tab, 'radio_batch'),
                    'mode_single': self._get_checkbox_checked(self.detect_tab, 'radio_mode_single'),
                    'mode_single_label': self._get_checkbox_checked(self.detect_tab, 'radio_mode_single_label'),
                    'mode_dual': self._get_checkbox_checked(self.detect_tab, 'radio_mode_dual'),
                    'device': self._get_combobox_text(self.detect_tab, 'combo_device'),
                },
                'train': {
                    'model_path': self._get_lineedit_text(self.train_tab, 'edit_model_path_train'),
                    'data_yaml': self._get_lineedit_text(self.train_tab, 'edit_yaml_path'),
                    'project': self._get_lineedit_text(self.train_tab, 'edit_project_dir'),
                    'name': self._get_lineedit_text(self.train_tab, 'edit_experiment_name', 'exp'),
                    'epochs': self._get_spinbox_value(self.train_tab, 'spin_epochs', 100),
                    'imgsz': self._get_spinbox_value(self.train_tab, 'spin_imgsz_train', 640),
                    'workers': self._get_spinbox_value(self.train_tab, 'spin_workers', 8),
                    'batch': self._get_spinbox_value(self.train_tab, 'spin_batch', 16),
                    'device': self._get_combobox_text(self.train_tab, 'combo_device_train'),
                    'hflip': self._get_doublespinbox_value(self.train_tab, 'spin_hflip', 0.5),
                    'vflip': self._get_doublespinbox_value(self.train_tab, 'spin_vflip', 0.0),
                    'degrees': self._get_doublespinbox_value(self.train_tab, 'spin_degrees', 0.0),
                    'mosaic': self._get_doublespinbox_value(self.train_tab, 'spin_mosaic', 1.0),
                    'mixup': self._get_doublespinbox_value(self.train_tab, 'spin_mixup', 0.0),
                    'hsv_h': self._get_doublespinbox_value(self.train_tab, 'spin_hsv_h', 0.015),
                    'hsv_s': self._get_doublespinbox_value(self.train_tab, 'spin_hsv_s', 0.7),
                    'hsv_v': self._get_doublespinbox_value(self.train_tab, 'spin_hsv_v', 0.4),
                    'box': self._get_doublespinbox_value(self.train_tab, 'spin_box_loss', 7.5),
                    'cls': self._get_doublespinbox_value(self.train_tab, 'spin_cls_loss', 0.5),
                    'dfl': self._get_doublespinbox_value(self.train_tab, 'spin_dfl_loss', 1.5),
                    'cache': self._get_checkbox_checked(self.train_tab, 'check_cache'),
                    'patience': self._get_spinbox_value(self.train_tab, 'spin_patience', 100),
                },
                'format': {
                    'model_path': self._get_lineedit_text(self.format_tab, 'edit_model_path_format'),
                    'format_onnx': self._get_checkbox_checked(self.format_tab, 'radio_onnx', True),
                    'half': self._get_checkbox_checked(self.format_tab, 'check_half'),
                    'device': self._get_combobox_text(self.format_tab, 'combo_device_format'),
                    'imgsz': self._get_spinbox_value(self.format_tab, 'spin_imgsz_format', 640),
                }
            }
    ...
    def load_config(self):
        ...
        try:
            ...
            self._set_lineedit(self.detect_tab, 'edit_model1', detect_cfg.get('model1', ''))
            self._set_lineedit(self.detect_tab, 'edit_model2', detect_cfg.get('model2', ''))
            self._set_lineedit(self.detect_tab, 'edit_img_path', detect_cfg.get('img_dir', ''))
            self._set_lineedit(self.detect_tab, 'edit_label_path', detect_cfg.get('label_dir', ''))
            self._set_doublespinbox(self.detect_tab, 'spin_conf', detect_cfg.get('conf', 0.25))
            self._set_doublespinbox(self.detect_tab, 'spin_iou', detect_cfg.get('iou', 0.45))
            self._set_spinbox(self.detect_tab, 'spin_imgsz', detect_cfg.get('imgsz', 640))
            self._set_spinbox(self.detect_tab, 'spin_max_det', detect_cfg.get('max_det', 300))
            self._set_lineedit(self.detect_tab, 'edit_classes', detect_cfg.get('classes', ''))
            self._set_spinbox(self.detect_tab, 'spin_quantize', detect_cfg.get('quantize', 0))
            self._set_checkbox(self.detect_tab, 'check_augment', detect_cfg.get('augment', False))
            self._set_checkbox(self.detect_tab, 'check_stream', detect_cfg.get('stream', False))
            # 单选按钮
            self._set_checkbox(self.detect_tab, 'radio_batch', detect_cfg.get('batch_mode', False))
            self._set_checkbox(self.detect_tab, 'radio_single', not detect_cfg.get('batch_mode', False))
            self._set_checkbox(self.detect_tab, 'radio_mode_single', detect_cfg.get('mode_single', True))
            self._set_checkbox(self.detect_tab, 'radio_mode_single_label', detect_cfg.get('mode_single_label', False))
            self._set_checkbox(self.detect_tab, 'radio_mode_dual', detect_cfg.get('mode_dual', False))
            self._set_combobox(self.detect_tab, 'combo_device', detect_cfg.get('device', 'cpu'))

            # 如果图片目录已设置，加载图片列表
            if self._get_lineedit_text(self.detect_tab, 'edit_img_path'):
                # 此处需要调用 DetectTab 的方法，但若从 .ui 加载则没有该方法，需要处理
                if isinstance(self.detect_tab, DetectTab):
                    self.detect_tab.load_image_list()

            # 恢复 train 标签页
            train_cfg = config.get('train', {})
            if train_cfg:
                self._set_lineedit(self.train_tab, 'edit_model_path_train', train_cfg.get('model_path', ''))
                self._set_lineedit(self.train_tab, 'edit_yaml_path', train_cfg.get('data_yaml', ''))
                self._set_lineedit(self.train_tab, 'edit_project_dir', train_cfg.get('project', ''))
                self._set_lineedit(self.train_tab, 'edit_experiment_name', train_cfg.get('name', 'exp'))
                self._set_spinbox(self.train_tab, 'spin_epochs', train_cfg.get('epochs', 100))
                self._set_spinbox(self.train_tab, 'spin_imgsz_train', train_cfg.get('imgsz', 640))
                self._set_spinbox(self.train_tab, 'spin_workers', train_cfg.get('workers', 8))
                self._set_spinbox(self.train_tab, 'spin_batch', train_cfg.get('batch', 16))
                self._set_combobox(self.train_tab, 'combo_device_train', train_cfg.get('device', 'cpu'))
                self._set_doublespinbox(self.train_tab, 'spin_hflip', train_cfg.get('hflip', 0.5))
                self._set_doublespinbox(self.train_tab, 'spin_vflip', train_cfg.get('vflip', 0.0))
                self._set_doublespinbox(self.train_tab, 'spin_degrees', train_cfg.get('degrees', 0.0))
                self._set_doublespinbox(self.train_tab, 'spin_mosaic', train_cfg.get('mosaic', 1.0))
                self._set_doublespinbox(self.train_tab, 'spin_mixup', train_cfg.get('mixup', 0.0))
                self._set_doublespinbox(self.train_tab, 'spin_hsv_h', train_cfg.get('hsv_h', 0.015))
                self._set_doublespinbox(self.train_tab, 'spin_hsv_s', train_cfg.get('hsv_s', 0.7))
                self._set_doublespinbox(self.train_tab, 'spin_hsv_v', train_cfg.get('hsv_v', 0.4))
                self._set_doublespinbox(self.train_tab, 'spin_box_loss', train_cfg.get('box', 7.5))
                self._set_doublespinbox(self.train_tab, 'spin_cls_loss', train_cfg.get('cls', 0.5))
                self._set_doublespinbox(self.train_tab, 'spin_dfl_loss', train_cfg.get('dfl', 1.5))
                self._set_checkbox(self.train_tab, 'check_cache', train_cfg.get('cache', False))
                self._set_spinbox(self.train_tab, 'spin_patience', train_cfg.get('patience', 100))

            # 恢复 format 标签页
            format_cfg = config.get('format', {})
            self._set_lineedit(self.format_tab, 'edit_model_path_format', format_cfg.get('model_path', ''))
            self._set_checkbox(self.format_tab, 'radio_onnx', format_cfg.get('format_onnx', True))
            self._set_checkbox(self.format_tab, 'radio_engine', not format_cfg.get('format_onnx', True))
            self._set_checkbox(self.format_tab, 'check_half', format_cfg.get('half', False))
            self._set_combobox(self.format_tab, 'combo_device_format', format_cfg.get('device', 'cpu'))
            self._set_spinbox(self.format_tab, 'spin_imgsz_format', format_cfg.get('imgsz', 640))
        except Exception as e:
            print(f"加载配置失败: {e}")
...
if __name__ == '__main__':
    ...
    if not ULTRALYTICS_AVAILABLE:
        QMessageBox.warning(None, "警告", "未检测到ultralytics库，请先安装: pip install ultralytics")
    window = MainWindow()
    window.show()
    sys.exit(app.exec_())