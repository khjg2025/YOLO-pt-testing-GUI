...
class TrainWorker(QThread):
    ...
    def run(self):
        try:
            ...
            device = self.params.get('device', 'cpu')
            hflip = self.params.get('fliplr', 0.5)
            vflip = self.params.get('flipud', 0.0)
...
class TrainTab(QWidget):
    ...
    def get_train_params(self):
        return {
            ...
            'device': self.combo_device.currentText(),
            'fliplr': self.spin_hflip.value(),
            'flipud': self.spin_vflip.value(),
    ...
    def import_config(self):
        ...
        try:
            ...
            self.spin_hflip.setValue(params.get('fliplr', 0.5))
            self.spin_vflip.setValue(params.get('flipud', 0.0))
...
class MainWindow(QMainWindow):
    ...
    UI_FILE = "_main_window.ui"  # 可选的UI文件
    ...
    def save_config(self):
        # 如果使用.ui加载，可能需要从控件获取值；这里简化处理，仅当使用代码构建时才保存
        if hasattr(self, 'detect_tab') and hasattr(self, 'train_tab') and hasattr(self, 'format_tab'):
            config = {
                ...
                'format': {
                    'model_path': self.format_tab.edit_model_path.text(),
                    'format_onnx': self.format_tab.radio_onnx.isChecked(),
                    'half': self.format_tab.check_half.isChecked(),
                    'device': self.format_tab.combo_device.currentText(),
                    'imgsz': self.format_tab.spin_imgsz.value(),
                }
            }
            try:
    ...
    def load_config(self):
        ...
        try:
            ...
            if hasattr(self, 'detect_tab'):
                ...
                self.detect_tab.spin_quantize.setValue(detect_cfg.get('quantize', 16))
            ...
            if hasattr(self, 'train_tab'):
                ...
                if train_cfg:
                    self.train_tab.edit_model_path.setText(train_cfg.get('model_path', ''))
                    ...
                    self.train_tab.spin_imgsz.setValue(train_cfg.get('imgsz', 640))
                    ...
                    idx = self.train_tab.combo_device.findText(dev)
                    if idx >= 0:
                        self.train_tab.combo_device.setCurrentIndex(idx)
                    self.train_tab.spin_hflip.setValue(train_cfg.get('fliplr', 0.5))
                    self.train_tab.spin_vflip.setValue(train_cfg.get('flipud', 0.0))
            ...
            if hasattr(self, 'format_tab'):
                ...
                self.format_tab.edit_model_path.setText(format_cfg.get('model_path', ''))
                ...
                idx = self.format_tab.combo_device.findText(dev)
                if idx >= 0:
                    self.format_tab.combo_device.setCurrentIndex(idx)
                self.format_tab.spin_imgsz.setValue(format_cfg.get('imgsz', 640))
        except Exception as e:
            print(f"加载配置失败: {e}")
